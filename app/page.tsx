...
export default function Home() {
  ...
  async function handleRetouch() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setAppState('done');
        // 사용량 갱신
        if (data.usageInfo) setUsageCount(data.usageInfo.used);
      } else {
  ...
  const isLimitExceeded = user ? usageCount >= FREE_LIMIT : false;
  ...
  return (
    <main className="min-h-screen" style={{ background: '#F4F1FA' }}>
      ...
      <div className="max-w-lg mx-auto px-4 py-10 space-y-5">
        ...
        {!authLoading && (
          <div
            className="rounded-[24px] p-4 flex items-center justify-between gap-3"
            style={{
            ...
            {user ? (
              <>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px]" style={{ color: isLimitExceeded ? '#ef4444' : '#635F69' }}>
                      {isLimitExceeded
                        ? '무료 횟수 소진 — 업그레이드 필요'
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: 'inset 3px 3px 6px rgba(180,170,200,0.15), inset -3px -3px 6px rgba(255,255,255,0.8)',
                    }}
                  >
                    로그아웃
                  </button>
              ...
              <>
                ...
                <button
                  ...
                  className="px-4 py-2.5 rounded-full text-xs font-black text-white transition-all active:scale-95"
                  style={{
                    background: 'linear-gradient(135deg, #A78BFA, #7C3AED)',
                    boxShadow: '8px 8px 16px rgba(139,92,246,0.25), -4px -4px 8px rgba(255,255,255,0.6)',
        ...
        {hasFile && !isProcessing && appState !== 'done' && (
          <RetouchControls
            ...
            disabled={isProcessing || isLimitExceeded}
          />
        )}
        ...
        {isLimitExceeded && hasFile && appState !== 'done' && (
          <div
            className="rounded-[28px] p-5 text-center space-y-3"
            style={{
              background: 'rgba(255,255,255,0.7)',
          ...
          >
            ...
            <p className="text-sm" style={{ color: '#635F69' }}>
              더 많이 사용하려면 업그레이드가 필요해요
            </p>
            <div
              className="inline-block px-5 py-3 rounded-full text-white text-sm font-black"
              style={{
                background: 'linear-gradient(135deg, #A78BFA, #7C3AED)',
          ...
          >
            ...
            <div className="flex-1">
              ...
              {errorCode === 'UNAUTHORIZED' && (
                <button
                  onClick={() => { setShowAuthModal(true); setAppState('idle'); }}
              ...
              {errorCode !== 'UNAUTHORIZED' && errorCode !== 'LIMIT_EXCEEDED' && (
